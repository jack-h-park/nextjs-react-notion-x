...
- Native chat (`pages/api/native_chat.ts`) consults `loadChatModelSettings` (`lib/server/chat-settings.ts`) to produce a runtime snapshot; that snapshot later drives streaming via the selected local client, along with metadata such as telemetry tags and guardrail decisions. Reverse RAG reuses the same snapshot so that enhancements stay in lockstep with the primary LLM.
...
- `requireLocal=true` means **no cloud LLM is allowed**. If `localBackendAvailable` is false, `pages/api/native_chat.ts` short-circuits with a 500 error, and logs highlight `localBackendAvailable=false`/`engine=local-*`. Indirect overrides such as header/query-backend still respect this gate because they feed into `loadChatModelSettings`.
...
- **`LOCAL_LLM_BACKEND` misconfigured or not set**
  - How it shows up: every `requireLocal=true` request immediately returns 500 because `loadChatModelSettings` cannot instantiate `LocalLlmClient` and `pages/api/native_chat.ts` sees `localBackendAvailable=false`. The matrix test prints errors for `unset`/`invalid` overrides.
  - How to fix: mirror the exact string (`ollama` or `lmstudio`), restart the service if necessary, and re-run `pnpm test:llm:matrix`; check `lib/local-llm/index.ts` to ensure env parsing is not failing.
- **Ollama/LM Studio process unreachable**
  - How it shows up: logs show `localBackendAvailable=false`, the handler logs `local backend required but not available`, and the UI may show a 500 banner for the preset. `loadChatModelSettings` will fall back to cloud only if `requireLocal=false`.
  - How to fix: start/restart the respective service (default URLs shown in `lib/local-llm/index.ts`), confirm `getLocalLlmClient` no longer throws, and verify the development log in `pages/api/native_chat.ts` reports `localBackendAvailable=true`.
