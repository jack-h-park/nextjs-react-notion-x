...
- **Tests**
  - `pnpm test:llm:matrix` (scripts/smoke/test-local-llm-matrix.ts) hits `/api/native_chat` with `requireLocal=true` by default, varying headers: expect HTTP 200 for `ollama`/`lmstudio` when those backends run, and HTTP 503 for `unset`/`invalid` overrides because the preset requires local.
