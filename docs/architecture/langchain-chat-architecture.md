...
- `langchain_chat_impl_heavy.ts` orchestrates LangChain (`@langchain/core`) only after all guardrail, telemetry, and cache decisions have been made. It builds:
  - Guardrail config & runtime settings (`getChatGuardrailConfig`, `loadChatModelSettings`, `sanitizeChatSettings`).
...
### Retrieval cache determinism
- The retrieval cache key is computed *after* the auto/multi (HyDE/rewrite) decision and reused for both reads and writes, avoiding the previous situation where an auto-generated alt query salted the write key and made identical requests miss. This keeps `retrieval_cache_hit` accurate while still allowing the latency savings of context reuse.
