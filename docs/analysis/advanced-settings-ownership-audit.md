...
1.  **Auto RAG Conflict:** Enabling "Reverse RAG" or "HyDE" in the UI **disables** the automatic quality check logic. The system assumes a user override implies "force this strategy," bypassing the "is retrieval weak?" heuristic.
...
## 2. Design Decisions & Policy (Applied)

Following review, the following policies are now authoritative:

1.  **Auto Mode is Sovereign & Explicit:**
    - **Invariant:** Strategy execution is decided by `shouldRun = forced || (autoConsider && isWeakRetrieval)`.
    - **Capability vs Force:** "Enabled" in settings only grants _capability_ (`autoConsider`). It never implies _force_.
...
- **(D) System/Auto:** Controlled by runtime heuristics (Auto RAG).
...
| **RAG: Reverse**    | Toggle | **Direct Override**         | **Disables Auto RAG** | High | **D** | **Conflict.** If checked, Auto RAG logic is bypassed. Should be System-controlled or "Start with Deep Search" action, not persistent setting.          |
| **RAG: HyDE**       | Toggle | **Direct Override**         | **Disables Auto RAG** | High | **D** | **Conflict.** Same as Reverse RAG.                                                                                                                     |
...
- **Scenario:** Admin enables "Auto RAG" to smartly use query rewriting only when necessary (saving latency).
- **User Action:** User checks "Reverse RAG" in settings because they "want better search."
...
- **UI Change:** Remove persistent "Reverse RAG" and "HyDE" toggles from the "Settings" drawer.
- **Replacement:**
...
1.  **Refactor UI:** Replace numeric sliders (TopK, Similarity) with a simplified "Retrieval Mode" selector:
    ...
    - **Deep Research** (High TopK, Auto RAG Aggressive)
