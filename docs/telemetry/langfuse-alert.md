...
4. Langfuse sampling and filters ensure only knowledge traffic is exported (chitchat is sampled or gated), so alert behavior remains stable across environments.
...
**Why this metric**  
- Dashboard C tile 3 monitors p99 latency so on-call knows when tail latency becomes the dominant user-impact signal.
- Tile 2 (p50) and Tiles 5/6 (retrieval vs LLM latency) contextualize whether the tail is driven by caching, retrieval, or provider issues.
...
**Common causes**
- Retrieval slowdowns caused by index growth, outdated rerankers, or vector DB latency.
- LLM provider throttling, cold-start penalties, or streaming timeouts.
- Oversized prompts/contexts that push LLM latency higher than historical baselines.
...
**Why this metric**  
- Dashboard C tile 4 monitors the abort rate for knowledge traffic, revealing client-side cancellations and server stream misses before user frustration peaks.
- Coupling this signal with tile 2/3 latency trends helps distinguish latency-induced aborts from other failure modes.
...
**Common causes**
- Client timeout thresholds or retries that kick in as latency crosses the fixed baseline.
- Gateway/distribution failures that close connections midstream and emit `aborted=true`.
- Streaming interruptions or guardrails that cancel requests when LLM providers emit errors.
...
**Why this metric**
- Dashboard C tile 7 (cache hit rate) exposes when cache leverage deteriorates, while tile 8 tracks hit vs miss latency parity, tying directly to the cost/latency lever Alert C monitors.
- These signals help confirm that cache misbehavior—not general traffic volume—is the root cause when latency or abort alerts also fire.
...
**Common causes**
- Vector database or retriever service outages that prevent responses from being cached.
- Cache TTL misconfigurations or invalidations that purge entries before reuse.
- Metadata mismatches (e.g., `intent`/`preset`) causing cache lookups to miss expected hits.
...
**Common causes**
- Cache metrics that report hits but lack `response_cache_hit` instrumentation, causing false parity signals.
- Retrieval latency regression for cache misses leaking into the hit cohort due to tagging inconsistencies.
- System-wide throttling or network errors that hit both cache hits and misses equally.
