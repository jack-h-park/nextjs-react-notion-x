...
## Phase 4 Notes – Scoped Dark Borders
- Phase 4 layered a page-scoped dark-mode override (`components/admin/layout/admin-ingestion-shell.module.css`) that pushes all `.ai-card` borders inside the ingestion shell to `--ai-role-border-base` when `.dark` or `.dark-mode` is active.  
- We intentionally avoid editing `styles/ai-design-system.css` because the broader token strategy should remain low-contrast; the stronger border exists only on this data-dense dashboard to keep edges perceptible.  
- Inset panels still rely on the global token contract, so this exception keeps the card surfaces coherent while only the Admin Ingestion route benefits from the stronger border.
