...
void test("requireLocal enforcement resolves to local_ok when backend available", () => {
...
void test("requireLocal enforcement blocks when backend missing", () => {
  const result = resolveRequireLocalEnforcement(true, true, false);
  assert.equal(result.enforcement, "blocked_require_local");
  assert.equal(result.shouldFallbackToCloud, false);
});

void test("requireLocal=false falls back to cloud when backend missing", () => {
...
void test("cloud preset with requireLocal=false remains cloud_ok", () => {
