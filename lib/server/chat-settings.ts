...
const LANGFUSE_PROVIDER_METADATA_SETTING_KEY =
  "langfuse_attach_provider_metadata";
...
export async function loadSystemPrompt(options?: {
  ...
  const prompt = data?.value
    ? normalizeSystemPrompt(data.value)
    : DEFAULT_SYSTEM_PROMPT;
...
export async function loadChatModelSettings(options?: {
  ...
  const embeddingModelSetting =
    settingsMap.get(CHAT_EMBEDDING_MODEL_SETTING_KEY) ?? null;
  const embeddingSpaceSetting =
    settingsMap.get(CHAT_EMBEDDING_SPACE_SETTING_KEY) ?? null;
  ...
  const result: ChatModelSettings = {
    ...
    isDefault: {
      ...
      llmModel: !llmModelSetting || llmSelection.id === defaultLlm.id,
      embeddingModel:
        (!embeddingModelSetting ||
          embeddingSelection.embeddingModelId ===
            defaultEmbedding.embeddingModelId) &&
        (!embeddingSpaceSetting ||
          embeddingSelection.embeddingSpaceId ===
            defaultEmbedding.embeddingSpaceId),
      reverseRagEnabled:
        reverseRagEnabledSetting === DEFAULT_REVERSE_RAG_ENABLED,
      reverseRagMode: reverseRagModeSetting === DEFAULT_REVERSE_RAG_MODE,
      hydeEnabled: hydeEnabledSetting === DEFAULT_HYDE_ENABLED,
      rankerMode: rankerModeSetting === DEFAULT_RANKER_MODE,
      embeddingSpaceId:
        !embeddingSpaceSetting ||
        embeddingSelection.embeddingSpaceId ===
          defaultEmbedding.embeddingSpaceId,
...
export async function saveChatModelSettings(
  ...
  const reverseRagModeValue = input.reverseRagMode ?? DEFAULT_REVERSE_RAG_MODE;
  ...
  const payload = [
    ...
    {
      key: CHAT_EMBEDDING_PROVIDER_SETTING_KEY,
      value: embeddingSelection.provider,
    },
    { key: CHAT_LLM_MODEL_SETTING_KEY, value: llmSelection.id },
    {
      key: CHAT_EMBEDDING_MODEL_SETTING_KEY,
      value: embeddingSelection.embeddingModelId,
    },
    {
      key: CHAT_EMBEDDING_SPACE_SETTING_KEY,
      value: embeddingSelection.embeddingSpaceId,
    },
  ];
  ...
  if (error) {
    ...
    console.error(
      "[chat-settings] failed to persist chat model settings",
      error,
    );
...
export async function saveGuardrailSettings(
  ...
  if (error) {
    ...
    console.error(
      "[chat-settings] failed to persist guardrail settings",
      error,
    );
...
export async function loadLangfuseSettings(options?: {
  ...
  if (error) {
    ...
    console.error("[chat-settings] failed to load Langfuse settings", error);
...
export async function saveLangfuseSettings(
  ...
  const devRate = clampSampleRate(
    input.sampleRateDev,
    DEFAULT_LANGFUSE_SAMPLE_RATE_DEV,
  );
  ...
  if (error) {
    ...
    console.error("[chat-settings] failed to persist Langfuse settings", error);
...
function buildLangfuseSettings(map?: Map<string, string>): LangfuseSettings {
  const envOverride = map?.get(LANGFUSE_ENV_SETTING_KEY);
  const sampleRateDevOverride = map?.get(LANGFUSE_SAMPLE_RATE_DEV_SETTING_KEY);
  const sampleRatePreviewOverride = map?.get(
    LANGFUSE_SAMPLE_RATE_PREVIEW_SETTING_KEY,
  );
  const metadataOverride = map?.get(LANGFUSE_PROVIDER_METADATA_SETTING_KEY);

  const envTag = envOverride?.trim().length
    ? envOverride.trim()
...
function buildGuardrailResult(overrides?: {
  keywords?: string;
  fallbackChitchat?: string;
  fallbackCommand?: string;
  ...
  const fallbackChitchatSource = normalizeOptionalValue(
    overrides?.fallbackChitchat,
  );
  const fallbackCommandSource = normalizeOptionalValue(
...
function buildNumericSettings(
  overrides?: Partial<
    Record<keyof GuardrailNumericSettings, string | undefined>
  >,
): {
  ...
  const similarity = resolveNumericSetting(
  ...
  const summaryEnabled = resolveBooleanSetting(
    overrides?.summaryEnabled,
...
function validateNumericInput(
  ...
  return {
    ...
    ragTopK: ensureMin(
      Math.round(ensureNumberField(input.ragTopK, "RAG top K")),
      1,
    ),
    ragContextTokenBudget: ensureMin(
      Math.round(
        ensureNumberField(input.ragContextTokenBudget, "Context token budget"),
      ),
      200,
    ),
    ragContextClipTokens: ensureMin(
      Math.round(
        ensureNumberField(input.ragContextClipTokens, "Context clip tokens"),
      ),
      64,
    ),
    historyTokenBudget: ensureMin(
      Math.round(
        ensureNumberField(input.historyTokenBudget, "History token budget"),
      ),
      200,
    ),
    ...
    summaryTriggerTokens: ensureMin(
      Math.round(
        ensureNumberField(input.summaryTriggerTokens, "Summary trigger tokens"),
      ),
      200,
    ),
