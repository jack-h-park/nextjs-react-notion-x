...
import { getOllamaRuntimeConfig, type OllamaRuntimeConfig } from '@/lib/core/ollama'
...
export async function* streamOllamaChat(
  ...
  if (!resolvedModel) {
    throw new OllamaUnavailableError('No Ollama model is configured. Set OLLAMA_MODEL_DEFAULT or provide a model id.')
  }
  ...
  try {
    ...
    while (true) {
      ...
      const { remainder, chunks } = drainBuffer(buffer, config.baseUrl, resolvedModel)
      buffer = remainder
    ...
    const { chunks: finalChunks } = drainBuffer(buffer, config.baseUrl, resolvedModel, true)
...
function buildMessages(systemPrompt: string, messages: ChatMessage[]) {
  ...
  const result: Array<{ role: 'system' | ChatMessage['role']; content: string }> = []
...
function normalizeModel(candidate: string | null | undefined, fallback: string) {
...
function processLine(line: string, baseUrl: string, model: string): string | null {
