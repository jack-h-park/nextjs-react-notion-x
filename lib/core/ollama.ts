...
const DEFAULT_MODEL = (process.env.OLLAMA_MODEL_DEFAULT ?? 'mistral').trim() || 'mistral'
const DEFAULT_TIMEOUT = 30_000

const rawBaseUrl = process.env.OLLAMA_BASE_URL?.trim() ?? ''
const nodeEnv = process.env.NODE_ENV ?? 'development'
const enableInProd = (process.env.OLLAMA_ENABLE_IN_PROD ?? 'false').toLowerCase() === 'true'
const resolvedBaseUrl = rawBaseUrl.length > 0 ? rawBaseUrl : nodeEnv === 'production' ? '' : DEFAULT_BASE_URL
const timeoutMs = parseEnvNumber(process.env.OLLAMA_TIMEOUT_MS, DEFAULT_TIMEOUT)
...
function parseEnvNumber(value: string | undefined, fallback: null): number | null
function parseEnvNumber(value: string | undefined, fallback: number | null): number | null {
  if (!value) {
    return fallback
  }
